...
estimated by \pkg{MixAll} with the \code{clusterPredict} function.
...
Default value is \code{"EM"}. Note that this default value is faster but
can produce a biased estimator.
...
replaces the {\bf I} step by a simulation step ({\bf S} step) and is not detailed
