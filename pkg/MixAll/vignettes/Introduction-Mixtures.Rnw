...
%---------------------------------------------------------
\title{Generative Clustering with missing values using the \pkg{MixAll} package}
\Plaintitle{Generative Clustering with missing values using the MixAll package}
\Shorttitle{Generative Clustering using the \pkg{MixAll} package}
...
\Abstract{
  ...
  heterogeneous data. Moreover, if there is mising values in the original
...
There is four kind of generative models implemented:
\begin{enumerate}
...
In this paper we will first present the mixture models and the different
algorithms, initialization methods and strategies that can be used in order to
estimate parameters of mixture models (Section~\ref{sec:MixtureTools}). In
Section~\ref{sec:ImplementedModels} we present the different mixture models
implemented in \proglang{STK++} that can be estimated using MixAll. Finally we
give examples of clustering on real data set in
Section~\ref{sec:ClustWithMixAll}.

\section{Mixture Modeling Estimation Tools}
...
the mean $\sum_{r=1}^R \theta^r/R$ of the iterates values.

At the end of the algorithm, an estimator of the missing values will be returned
by computing either, the averaged value of the $(\bx_i^m)$ (for quantitative
variables) or the most probable value (for nominal variables).
...
At the end of the algorithm, an estimator the missing values will be returned
by computing either, the averaged value of the $(\bx_i^m)$ (for quantitative
variables) or the most probable value (for nominal variables).
...
\subsection{Creating an algorithm in MixAll}\label{subsec:CretingAlgo}

All the algorithms (EM, \code{SEM}, \code{CEM} and \code{SemiSEM}) are encoded
in a \code{S4} class and can be created using the utility function
...
initialization with a limited number of an algorithm are performed and
...
initialization step. Possible values are \code{"EM"}, \code{"SEM"}, \code{"CEM"}, \code{"SemiSEM"}. Default value is \code{"EM"}.\\
\hline
...
(Remember the strategy launch an initialization before each short run). Default
value is 5.\\
\hline
\code{shortRunAlgo} & String with the estimation algorithm to use in short run(s). Possible values are \code{"EM"}, \code{"SEM"}, \code{"CEM"}, \code{"SemiSEM"}. Default value is \code{"EM"}.\\
\hline
...
\code{nbShortRun} starting points of the algorithm.
...
\begin{itemize}
\item the \code{clusterFastStrategy} for the impatient user,
\item the \code{clusterSemiSEMStrategy} for user with missing values.
\end{itemize}

For impatient user, the \code{clusterFastStrategy} will furnish quickly
results. The accuracy of the results is not guaranteed if the model is a bit
difficult to estimate.
<< >>=
clusterFastStrategy()
@

The function \code{clusterSemiSEMStrategy} is highly recommended if the user want a
non-biased estimator of the missing values. "SemiSEM" algorithm
computes a Monte-Carlo estimator of the missing values during the iterations.
<< >>=
...
\label{subsec:DiagGaussianModels}
...
\label{subsec:CategoricalModels}
...
\item \code{clusterHeterogeneous} for Heterogeneous mixture models.
\end{enumerate}

Heterogeneous mixture model is a special model allowing to cluster heterogeneous
data sets assuming conditional independency. More precisely, assume
...
\begin{equation}
 ...
 = \sum_{k=1}^K p_k \prod_{l=1}^L h({\bx}_{li}| \blambda_{lk},\balpha_l).
\end{equation}
The density functions (or probability distribution functions) $h(.|
...
Input Parameter & Description \\
\hline
\code{nbCluster} & Numeric. A vector with the number of clusters in the
model(s). Default is 2.\\
\hline
...
Possible values: \code{BIC},  \code{ICL}. Default is \code{ICL}.\\
\hline
...
This function has one mandatory argument: a \code{matrix} or \code{data.frame} $\bx$.
In Table~\ref{tab:clusterDiagGaussian} the reader will find a summary of all
...
\hline
\code{modelNames} & A \code{vector} object defining the list of models to
estimate.  Call \code{clusterDiagGaussianNames()} by default
(see \ref{subsec:DiagGaussianModels}).\\
\hline
\end{tabular}
...
\code{data} & Matrix or data frame\\
\hline
\code{modelNames} & A \code{vector} defining the models to estimate. Call
\code{clusterCatgoricalNames()} by default (see \ref{subsec:CategoricalModels}).\\
\hline
\end{tabular}
\caption{List of all the specific parameters of the
...
\hline
\code{modelNames} & A \code{vector} defining the models to estimate. Call
\code{clusterGammaNames()} by default (see \ref{subsec:GammaModels}).\\
\hline
\end{tabular}
\caption{List of all the specific parameters of the
...
\hline
\code{modelNames} & A \code{vector} defining the models to estimate. Call
\code{clusterPoissonNames()} by default (see \ref{subsec:PoissonModels}).\\
\hline
\end{tabular}
\caption{List of all the specific parameters of the
...
\subsection{Clustering Heterogeneous data sets}
Heterogeneous data sets can be clustered using the \code{clusterHeterogeneous}
function. The original heterogeneous data set has to be splited in multiple
homogeneous data sets and to associate a mixture model name to each one.

In Table~\ref{tab:clusterHeterogeneous} the reader will find a summary of all
the specific input parameters of this function with its default value.
\begin{table}[htb]
\centering
\begin{tabular}{|p{0.3\textwidth}|p{0.65\textwidth}|}
...
\hline
\code{modelNames} & A \code{vector} of character and of same length than data
containing the model names to fit to each data set.\\
\hline
\end{tabular}
\caption{List of all the specific parameters of the \code{clusterHeterogeneous}
function.}
\label{tab:clusterHeterogeneous}
\end{table}
...
Heterogeneous mixture models are plotted using the \emph{logistic latent
representation}.
