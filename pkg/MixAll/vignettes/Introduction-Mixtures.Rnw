...
%\VignetteIndexEntry{MixAll-cluster}
%\VignetteKeywords{MixAll, Rcpp, C++, R}
...
the \pkg{MixAll} package}
\Plaintitle{Generative Clustering with missing
values using the MixAll package}
\Shorttitle{Generative Clustering using the \pkg{MixAll}
package}
...
\Abstract{
  ...
  process. These imputations can be biased or exact estimators of the Maximum
  A Posteriori (MAP) values depending of the algorithm used.
}
...
STK++ that can be used form MixAll. Finally we give some examples on various data
set.
...
\subsection{Defining an algorithm in MixAll}\label{subsubsec:algo}
