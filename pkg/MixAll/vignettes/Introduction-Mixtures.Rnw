...
%---------------------------------------------------------
\title{\pkg{MixAll}: Clustering Heterogenous data with Missing Values}
\Plaintitle{MixAll: Clustering Heterogenous data with Missing Values}
...
\begin{enumerate}
  ...
  \item the diagonal gamma mixture models (24 models), see sections
  \ref{subsec:Gamma} and \ref{subsec:clustGamma},
  \item the diagonal categorical mixture models (4 models), see sections
  \ref{subsec:Categorical} and \ref{subsec:clustCategorical},
  \item the diagonal Poisson mixture models (6 models), see sections
...
\end{enumerate}
and a special model called "heterogeneous" mixture model allowing to cluster
heterogeneous data set using conditional independance between the different
kind of data, see sections \ref{subsec:Heterogeneous} and
\ref{subsec:clustHeterogeneous}.
...
\subsection{Heterogeneous Mixture Models}
\label{subsec:Heterogeneous}

Heterogeneous mixture models are special models allowing to cluster
heterogeneous data sets assuming conditional independency. More precisely, assume
...
\label{subsec:clustHeterogeneous}
...
containing the model names to fit to each data set (see \ref{subsec:Heterogeneous}).\\
\hline
\end{tabular}
