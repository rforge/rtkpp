...
$\balpha$ are not dependant from $k$ and are common to all the components of the
...
concentrated around the maximaum likelihood parameter estimator. A natural
...
It is possible to predict cluster observations and to estimate
missing values based on a mixture model estimated by \pkg{MixAll}
using the \code{clusterPredict} function. If there is no missing data
the predicted cluster are computed using Maximum A Posteriori (MAP) estimator.

Given a new observation $\bx$ the Bayes classifier is the function
$$
