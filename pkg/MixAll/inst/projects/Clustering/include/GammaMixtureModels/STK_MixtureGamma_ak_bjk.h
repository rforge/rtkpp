...
/** @file STK_MixtureGamma_ak_bjk.h
 *  @brief In this file we define the Gamma_pk_ak_bjk and Gamma_p_ak_bjk models.
 **/

#ifndef STK_MIXTUREGAMMA_AK_BJK_H
#define STK_MIXTUREGAMMA_AK_BJK_H
...
namespace STK
{
template<class Array>class MixtureGamma_ak_bjk;

namespace hidden
{
/** @ingroup Clustering
 * Traits class for the MixtureGamma_ak_bjk traits policy
 **/
template<class Array_>
struct MixtureTraits< MixtureGamma_ak_bjk<Array_> >
{
  typedef Array_ Array;
  /** Type of the structure storing the parameters of a MixtureGamma_ak_bjk model*/
...
/** @ingroup Clustering
 *  Structure encapsulating the parameters of a MixtureGamma_ak_bjk model.
 */
template<>
...
/** @ingroup Clustering
 *  MixtureGamma_ak_bjk is a mixture model of the following form
 * \f[
...
class MixtureGamma_ak_bjk : public MixtureGammaBase< MixtureGamma_ak_bjk<Array> >
{
  public:
    typedef MixtureGammaBase< MixtureGamma_ak_bjk<Array> > Base;
    using Base::param_;
    using Base::p_data;
    using Base::meanjk;
     ...
     **/
    inline MixtureGamma_ak_bjk( int nbCluster): Base(nbCluster) {}
    /** copy constructor
     *  @param model The model to copy
     **/
    inline MixtureGamma_ak_bjk( MixtureGamma_ak_bjk const& model): Base(model) {}
    /** destructor */
    inline ~MixtureGamma_ak_bjk() {}
 ...
 */
template<class Array>
void MixtureGamma_ak_bjk<Array>::randomInit( CArrayXX const*  p_tik, CPointX const* p_nk) 
{
    // compute moments
    this->moments(p_tik);
...
template<class Array>
bool MixtureGamma_ak_bjk<Array>::run( CArrayXX const*  p_tik, CPointX const* p_nk) 
{
  ...
  for (int k= p_tik->beginCols(); k < p_tik->endCols(); ++k)
  {
    ...
    if (!Arithmetic<Real>::isFinite(a))
    {
        ...
        stk_cout << "ML estimation failed in MixtureGamma_ak_bjk::run( CArrayXX const*  p_tik, CPointX const* p_nk) \n";
        stk_cout << "x0 =" << x0 << _T("\n";);
...
#endif /* STK_MIXTUREGAMMA_AK_BJK_H */
