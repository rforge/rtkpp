...
/* Compute the weighted mean and the common variance. */
template<class Array>
bool Gamma_ajk_bj<Array>::paramUpdateStep()
{
  ...
  for(iter=0; iter<MAXITER; ++iter)
  {
    ...
    if (value < qvalue)
    {
      stk_cout << _T("In Gamma_ajk_bj::paramUpdateStep(): mStep diverge\n");
  ...
  if (iter == MAXITER)
  {
    stk_cout << _T("In Gamma_ajk_bj::paramUpdateStep(): mStep did not converge\n");
