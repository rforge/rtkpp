...
/** @file STK_MixtureGamma_ak_b.h
 *  @brief In this file we define the Gamma_pk_ak_b and Gamma_p_ak_b mixture models.
 **/


#ifndef STK_MIXTUREGAMMA_AK_B_H
#define STK_MIXTUREGAMMA_AK_B_H
...
namespace STK
{
template<class Array>class MixtureGamma_ak_b;

namespace hidden
{
/** @ingroup Clustering
 *  Traits class for the MixtureGamma_ak_b traits policy. */
template<class Array_>
struct MixtureTraits< MixtureGamma_ak_b<Array_> >
{
  typedef Array_ Array;
  /** Type of the structure storing the parameters of a MixtureGamma_ak_b model*/
...
/** @ingroup Clustering
 *  Structure encapsulating the parameters of a MixtureGamma_ak_b model.
 */
template<>
...
/** @ingroup Clustering
 *  MixtureGamma_ak_b is a mixture model of the following form
 * \f[
...
class MixtureGamma_ak_b : public MixtureGammaBase<MixtureGamma_ak_b<Array> >
{
  public:
    typedef MixtureGammaBase<MixtureGamma_ak_b<Array> > Base;
    using Base::param_;
     ...
     **/
    inline MixtureGamma_ak_b( int nbCluster): Base(nbCluster) {}
    /** copy constructor
     *  @param model The model to copy
     **/
    inline MixtureGamma_ak_b( MixtureGamma_ak_b const& model): Base(model) {}
    /** destructor */
    inline ~MixtureGamma_ak_b() {}
 ...
 */
template<class Array>
void MixtureGamma_ak_b<Array>::randomInit( CArrayXX const*  p_tik, CPointX const* p_nk) 
{
    // compute moments
    this->moments(p_tik);
  // simulates ak
...
/* Compute the weighted mean and the common variance. */
template<class Array>
bool MixtureGamma_ak_b<Array>::run( CArrayXX const*  p_tik, CPointX const* p_nk) 
{
  ...
  for(iter=0; iter<MAXITER; ++iter)
  {
    ...
    if (value < qvalue)
    {
      stk_cout << _T("In MixtureGamma_ak_b::run( CArrayXX const*  p_tik, CPointX const* p_nk) : run( CArrayXX const*  p_tik, CPointX const* p_nk)  diverge\n");
  ...
  if (iter == MAXITER)
  {
    stk_cout << _T("In MixtureGamma_ak_b::run( CArrayXX const*  p_tik, CPointX const* p_nk) : run( CArrayXX const*  p_tik, CPointX const* p_nk)  did not converge\n");
