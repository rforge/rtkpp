 ...
 */
template<class Array>
void Gamma_ak_bjk<Array>::randomInit( CArrayXX const*  p_tik, CPointX const* p_nk) 
{
    // compute moments
    this->moments(p_tik);
...
template<class Array>
bool Gamma_ak_bjk<Array>::run( CArrayXX const*  p_tik, CPointX const* p_nk) 
{
  ...
  for (int k= p_tik->beginCols(); k < p_tik->endCols(); ++k)
  {
    ...
    if (!Arithmetic<Real>::isFinite(a))
    {
        ...
        stk_cout << "ML estimation failed in Gamma_ak_bjk::run( CArrayXX const*  p_tik, CPointX const* p_nk) \n";
        stk_cout << "x0 =" << x0 << _T("\n";);
