%\VignetteIndexEntry{STK++ Statistical Utilities Reference Guide}
...
@
% Title Page
\title{STK++ Statistical Utilities Reference Guide}
...
\hline
Exponential      & \ttcode{Law::Exponential(lambda)}     & \ttcode{*exp}   & Parameterization $\lambda e^{-\lambda x}$ \\
\hline
...
backends for dense matrix matrix decompositions (QR, Svd, eigenvalues) and
least square regression. In order to use lapack, you must activate its usage by
defining the following macros \ttcode{-DSTKUSELAPACK} at compilation time and
by linking your code with your installed lapack library using \ttcode{-llapack}
