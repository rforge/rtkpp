%\VignetteIndexEntry{Introduction to arrays usage}
...
\lstdefinestyle{customcpp}{
  ...
  commentstyle=\ttfamily\footnotesize\color{green!40!black},
...
\newcommand{\code}[1]{\lstinline[style=customcpp]$#1$}
...
@
% Title Page
\title{ General Review of the STK++ Arrays and their usages}
...
\maketitle
\begin{abstract}
This vignette gives a general review of the STK++ arrays and of their uses.
After giving a general review of the capability of the arrays, we detail the various
accessors/mutators available. We give a complete list of the constructors.

\end{abstract}

\section{General Review of the STK++ Arrays and theirs usages}
...
\lstinputlisting[style=customcpp,caption=First
example]{programs/tutoAccessors.cpp}
\end{minipage}
\hspace{0.2cm}
...
\begin{minipage}[t]{0.66\textwidth}
\lstinputlisting[style=customcpp,caption=Accesors]{programs/tutoAccessors2.cpp}
\end{minipage}
\hspace{0.2cm}
\begin{minipage}[t]{0.33\textwidth}
\addtocounter{lstlisting}{-1}
...
\subsection{Templated Expression}
...
of the data that will be stored.

On the other hand arrays in \code{CArray} family have four template parameters:
\begin{enumerate}
...
\begin{note}
Many more examples can be found in the test files coming with the STK++ lbrary.
\end{note}
...
The \code{Array2D} is a family of array/vectors/points proposing many
functionalities allowing to add/remove rows/columns. It is also possible to
...
and modified using this "reference" container. Thsi is achieved
