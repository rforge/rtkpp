...
template <>
struct ParametersHandler<Clust::Gamma_aj_bk_>: public ParametersHandlerGammaBase
{
...
class Gamma_aj_bk : public GammaBase< Gamma_aj_bk<Array> >
{
  public:
    ...
    inline Real shapeImpl(int k, int j) const { return param_.shape_()[j];}
    /** @return the scale of the kth cluster and jth variable */
    inline Real scaleImpl(int k, int j) const { return param_.scale_[k];}
