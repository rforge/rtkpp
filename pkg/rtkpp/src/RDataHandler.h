...
/** @ingroup DManager
 *  The RDataHandler class allow to read various csv files with their description
...
class RDataHandler : public STK::IDataHandler
{
  public:
    ...
    /** default constructor */
    inline RDataHandler() : STK::IDataHandler()
    ...
    inline STK::ReadWriteCsv const& data() const { return data_;}
    /** get the whole descriptor set */
    inline STK::ReadWriteCsv const& descriptor() const { return descriptor_;}
    ...
    bool readDataFromArray2D(STK::Array2D<Type> const& data, std::string const& idMixture, std::string const& idModel);
  ...
  private:
    /** data files */
    STK::ReadWriteCsv data_;
    /** descriptor files */
    STK::ReadWriteCsv descriptor_;
};


template<typename Type>
bool RDataHandler::readDataFromArray2D(STK::Array2D<Type> const& data, std::string const& idMixture, std::string const& idModel)
{
  // add descriptor
  STK::Variable<std::string> desc(2);
  desc[0] = idMixture; desc[1] = idModel;
