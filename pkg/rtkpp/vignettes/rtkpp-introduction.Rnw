...
STK::RcppMatrix<double> mat(m_data); // STK::RcppMatrix<double> mat(data) will work
\end{lstlisting}
The templated class \texttt{STK::RcppMatrix} wrap the Rcpp matrix (which itself
wrap the \texttt{R} \verb+SEXP+ structure). You can access directly (and
eventually modify) the \texttt{R} data in your application.

The second templated class you can use is the \texttt{STK::RcppVector} which
...
{
  BEGIN_RCPP
  STK::RcppMatrix<double> m_data(r_matrix);
...
PKG_CPPFLAGS = `${R_HOME}/bin/Rscript -e "Rcpp:::CxxFlags()"` \
               `${R_HOME}/bin/Rscript -e "rtkpp:::CppFlags()"` \
